#!/usr/bin/env python
"""Figure S15: model-basedness from the fixed model, and its agreement with arbitration.

Panel A  Fixed-model W across generations, by condition (mean +/- 2 SEM).
Panel B  Fixed-model W against arbitration-model W, one facet per condition, with a
         least-squares fit and its R^2.

The two models estimate the same construct from the same choices by different means:
the fixed model fits model-basedness as a free parameter (separately for high- and
low-stake trials, averaged here), while the arbitration model computes it online from
the running performance of the two controllers. Panel B is therefore a
convergent-validity check, not a replication.

Input : data/processed/fixed_model_fitted_params.csv
        data/processed/model_basedness_comparison.csv
        data/processed/model_basedness_comparison_stats.csv
Output: figures/S15_model_basedness_comparison.{pdf,png}
"""
...
logger = logging.getLogger("figS15")

FIGURE_STEM = "S15_model_basedness_comparison"
FIXED_NAME = "fixed_model_fitted_params.csv"
COMPARISON_NAME = "model_basedness_comparison.csv"
...
def draw_generations(ax, fixed: pd.DataFrame) -> None:
    """Panel A: fixed-model W per generation, with a +/- 2 SEM band."""
    grouped = fixed.groupby(["condition", "generation"])["w_fixed"]
...
def draw_agreement(
    ax, group: pd.DataFrame, fit: pd.Series, condition: str, show_ylabel: bool
) -> None:
    """Panel B facet: fixed-model W against arbitration-model W."""
    style = CONDITION_STYLE[condition]
    x = group["w_arbitration"].to_numpy()
    y = group["w_fixed"].to_numpy()
    ...
    ax.set_xlabel("$W$ (Arbitration model)")
    if show_ylabel:
        ax.set_ylabel("$W$ (Fixed model)")
    else:
        ax.set_yticklabels([])
...
def main() -> int:
    ...
    comparison = pd.read_csv(paths.DATA_PROCESSED / COMPARISON_NAME)
    ...
    draw_generations(ax_generations, fixed)

    axes_agreement = []
    for offset, condition in enumerate(CONDITION_STYLE):
        shared = axes_agreement[0] if axes_agreement else None
        ax = fig.add_subplot(grid[0, 1 + offset], sharey=shared)
        fit = fits.loc[condition]
        draw_agreement(
            ax, comparison[comparison["condition"] == condition], fit, condition,
            show_ylabel=offset == 0,
        )
        logger.info("%-11s n=%4d  R2=%.3f  p=%.3g", condition, int(fit["n"]),
                    fit["r_squared"], fit["p_value"])
        axes_agreement.append(ax)

    for ax, label in zip([ax_generations, axes_agreement[0]], ["A", "B"]):
        ax.text(-0.18, 1.08, label, transform=ax.transAxes, fontsize=14,
                fontweight="bold", va="top", ha="left")

    for ax in [ax_generations, *axes_agreement]:
    ...
    for condition in CONDITION_STYLE:
        series = (
            fixed[fixed["condition"] == condition]
            .groupby("generation")["w_fixed"].mean().round(3).to_list()
        )
        logger.info("%-11s mean W by generation: %s", condition, series)
    return 0
