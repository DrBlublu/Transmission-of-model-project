...
def create_reward_distribution(n_trials):
    """"Create two reward distributions with gaussian random walk and reflecting boundaries"""
    val_high_start = random.randint(5, 9)
    val_low_start = random.randint(0, 5)

    L_val_step2_A = [val_high_start]
    L_val_step2_B = [val_low_start]
    
    for _ in range(n_trials - 1):
        new_val_step2_A = random_walk(L_val_step2_A[-1])
        L_val_step2_A.append(new_val_step2_A)
        
        new_val_step2_B = random_walk(L_val_step2_B[-1])
