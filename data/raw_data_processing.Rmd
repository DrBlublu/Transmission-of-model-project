...
transfer_file <- function(dump_filepath, filepath, study_id) {
  # Trasnfer files from the raw folder to the designed run folder.
  # This function load every file in the raw folder, check if they are
  # from a specific study_id (i.e., a run on Prolific), and if it's the case
  # move all these files in a proper folder "run/study_id".
  #
  # Args:
  #   dump_filepath: (str) filepath of all the raw datafile
  #   filepath: (str) filepath to send the raw file to
  #   study_id: (str) id of the Prolific study
  #
  # Returns:
  #   None
...
integrity_check <- function(df_raw) {
  # Check for integrity in questionnaire and task
  # param  : df_raw A tibble
  # return : A vector of two num
...
validity_check <- function(filepath, archive_filepath, treatment) {
  # Show message, check integrity of score / questionnaire, and ask if it's ok
  # to continue. In case it's not, move file to a archive folder
  # param  : filepath A chr
  # param  : archive_filepath A chr
  n_task <- 0
  n_questionnaire <- 0
...
random_file_selection <- function(filepath, non_select_filepath, n) {
  # Randomly select 100 file from a folder and move non-selected file to a
  # specific folder (only used if number of file > 100)
  # param  : filepath A chr
  # param  : non_select_filepath A chr
...
process_selection <- function(folder, output_filepath) {
  # Load file, correct score and create selection.csv file with rank, filename,
  # score and new condition filename (e.g., chain0.csv)
  # param  : folder A chr
  # param  : output_filepath A chr
  # return : a tibble
  
  filepath <- file.path(path, "raw_data", "run", folder)
...
process_cond_data <- function(df_next_gen, filepath, next_gen_filepath, treatment) {
  # Create cond file for the next generation based on df_next_gen
  # param : df_next_gen A tibble
  # param : filepath A chr
  # param : next_gen_filepath A chr
...
df_filename <- read.csv("../../experiment/03 ressources/stim_name_filename.csv")

filepath <- file.path(path, "raw_data", "run", run_name)
integrity_filepath <- file.path(path, "raw_data", "archived", "run", run_name, "integrity")
non_select_filepath <- file.path(path, "raw_data", "archived", "run", run_name, "not_selected")
dump_filepath <- file.path(path, "raw_data", "datadump")
next_gen_filepath <- file.path(path, "output", "next gen file", run_name)
...
filepath <- file.path("run", run_name)
integrity_filepath <- file.path("archived", "run", run_name, "integrity")
non_select_filepath <- file.path("archived", "run", run_name, "not_selected")
dump_filepath <- file.path("raw")
...
```{r}
transfer_file <- function(dump_filepath, filepath, study_id) {
  # Copy/paste file from raw folder to raw data run folder
  # param : dump_filepath A chr
  # param : filepath A chr
  # param : study_id A chr
  list_filename <- list.files(dump_filepath)
  for (i in 1:length(list_filename)) {
    filename <- list_filename[i]
    print(filename)
