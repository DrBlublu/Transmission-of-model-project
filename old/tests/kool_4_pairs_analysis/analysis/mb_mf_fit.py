...
def apply_priors(params,model_func_params):
	...
	"""
	LL = MB_MF_rllik_learn_mat_arms(params,model_func_params[0],model_func_params[1],model_func_params[2])
	...
	priors_total_ll = beta_ll + alpha_ll + lambda_ll + wlolo_ll + whilo_ll + wlohi_ll + whihi_ll +stick_ll + resp_stick_ll + eta_ll+ kappa_ll
	posterior_LL = LL - priors_total_ll
	return(posterior_LL)

def MB_MF_rllik_learn_mat_arms(params,sub_df,stakes=4,final=True):
	"""
	...
	for trial in range(0,N):
		...
		if (sub_df.loc[trial,'rt_1'] == -1  or sub_df.loc[trial,'rt_2'] == -1 ): # remove this and screen out rt == -1 in subject csvs instead
			continue
		...
		if stakes == 4:
			if sub_df.loc[trial,'stake'] == 1 and (sub_df.loc[trial,'state1']  not in environments):
				w = w_lo_low_arm 
			elif sub_df.loc[trial,'stake'] == 5 and (sub_df.loc[trial,'state1']  not in environments):
				w = w_hi_low_arm
			elif sub_df.loc[trial,'stake'] == 1 and (sub_df.loc[trial,'state1'] in environments):
				w = w_lo_high_arm 
			elif sub_df.loc[trial,'stake'] == 5 and (sub_df.loc[trial,'state1'] in environments):
				w = w_hi_high_arm
		elif stakes == 2:
		...
		Q = w*Qmb + (1-w)*np.atleast_2d(Qmf[s1-1,:]).T + st*np.atleast_2d(M[s1-1,:]).T + respst*R # have to do some weird things to change row vecs to column vecs
		...
		if final == False:
			...
			Tm[s1-1][abs(s2-3)-1, abs(a-3)-1] =  Tm[s1-1][abs(s2-3)-1, abs(a-3)-1] + (eta*kappa)*virtual_spe
		...
		dtQ[1] = sub_df.loc[trial,'points'] - Q2[s2-1] # prediction error for 2nd choice
